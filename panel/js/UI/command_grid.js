...
function addCommand(command_name, command_target_array, command_value, auto, insertCommand) {
    ...
    for (var i = 0; i < 4; ++i) {
        ...
        if (i == 0) {
            ...
            if("name:input" == command_target_array[command_target_array.length - 1][1]) {
                div_hidden.appendChild(document.createTextNode(command_target_array[command_target_array.length - 1][0]));
                div_show.appendChild(document.createTextNode(command_target_array[command_target_array.length - 1][0]));
            }
        }
