...
function fileToPanel(f) {
    ...
    if (tr)
        for (var i = 0; i < tr.length; ++i) {
            pattern = tr[i].match(/(?:<tr>)([\s]*?)(?:<td>)([\s\S]*?)(?:<\/td>)([\s]*?)(?:<td>)([\s\S]*?)(?:<datalist>)([\s\S]*?)(?:<\/datalist>([\s]*?)<\/td>)([\s]*?)(?:<td>)([\s\S]*?)(?:<\/td>)([\s]*?)(?:<td>)([\s\S]*?)(?:<\/td>)([\s]*?)(?:<td>)([\s\S]*?)(?:<\/td>)([\s]*?)(?:<\/tr>)/);
            if (pattern == null) {
            ...
            var new_tr = '<tr>'
                + pattern[1] + '<td><div style="display: none;">' + pattern[2] + '</div><div style="overflow:hidden;height:15px;"></div></td>'
                + pattern[3] + '<td><div style="display: none;">' + pattern[4] + '</div><div style="overflow:hidden;height:15px;"></div>\n        ' + '<datalist>' + pattern[5] + '</datalist>' + pattern[6] + '</td>'
                + pattern[7] + '<td><div style="display: none;">' + pattern[8] + '</div><div style="overflow:hidden;height:15px;"></div></td>'
                + pattern[9] + '<td><div style="display: none;">' + checkEmpty(pattern[10]) + '</div><div style="overflow:hidden;height:15px;"></div></td>'
                + checkEmpty(pattern[11])
                + '</tr>';
        ...
        if (count !== '0') {
            ...
            for (var i = 1; i <= count; ++i) {
                ...
                for (var j = 0; j < 4; ++j) {
                    ...
                    if (node.childNodes[1] != null && node.childNodes[0] != null && node.childNodes[0].innerHTML != null) {
