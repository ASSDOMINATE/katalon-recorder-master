        ...
        try {
            ...
            for (var j = 0; j < locatorResults.length; j++) {
                ...
                if (locator) {
                    ...
                    if (finderName != 'tac') {
                        var fe = this.findElement(locator);
                        if (isCustomFinderName(finderName) || (e == fe) || (coreLocatorStrategies[finderName] && coreLocatorStrategies[finderName].is_fuzzy_match && coreLocatorStrategies[finderName].is_fuzzy_match(fe, e))) {
                            locators.push([locator, finderName]);
                        }
                    } else {
...
function isCustomFinderName(name) {
    var customNames = ["name:input"];
...
LocatorBuilders.add('name:input', function (e) {
    if (e.nodeName != 'INPUT') {
        return null;
    }
    var prevNode = e.previousElementSibling;
    if (prevNode != null && (prevNode.nodeName == "LABEL" || prevNode.nodeName == "SPAN")) {
        return formatName(prevNode.innerText);
    }
    var parentNode = e.parentNode;
    if (parentNode.nodeName == "TD") {
        var parentPrevNode = parentNode.previousElementSibling;
        if (parentPrevNode != null && parentPrevNode.nodeName == "TD") {
            return formatName(parentPrevNode.innerText);
        }
    }
    return null;
});
...
function formatName(name) {
    var reg = /\\|\/|\?|\？|\*|\"|\“|\”|\'|\‘|\’|\<|\>|\{|\}|\[|\]|\【|\】|\：|\:|\、|\^|\$|\!|\~|\`|\|/g;
    if (name.charAt(name.length - 1).match(reg)) {
